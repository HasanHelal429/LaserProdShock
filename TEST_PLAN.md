...
- [x] `deck.py` emits a `collisions:` block, in `--verify` (§12.3)
- [x] `deck.py` emits a hybrid solver block, in `--verify` (§12.3)
- [x] `runs/P4/{README, P4_lez_kin, P4_lez_kin_off, P4_lez_hyb, P4_lez_flash}` written;
      all gates clean on the three WarpX legs; all three decks generate and round-trip
      through `--verify`; `tests/test_phase4_schema.py` (13 checks), 268 in the suite
...
- [ ] `P4_lez_kin_off` — G3 control, launches with its parent
...
plus `P4_lez_kin_off`, the G3 laser-off control for the kinetic leg. It is not optional
here: G2 reports `dz/λ_D` = 113 in the cold dense target, so numerical grid heating is a
live threat to `T_e` — the one quantity the entire benchmark turns on — and at ~0.09 GPU-h
it is the cheapest insurance in the phase.
...
### 12.3 RESOLVED 2026-08-12 — the tooling gaps are closed

Phase 4 could not be generated by the tooling as it stood. Two gaps, both real, **both now
implemented** (`tests/test_phase4_schema.py`, 13 checks; 268 in the suite):

1. **Collisions.** A `collisions:` block now maps to WarpX `collisions.*`
   (`pairwisecoulomb`, `CoulombLog`, `ndt_supercycle`), with intra-species pairs correctly
   emitted as a single species name. The paper is explicit that collisions are load-bearing
   — *"auxiliary simulations with either collisions or laser heating turned off
   demonstrated drastically different plasma evolution"*.
2. **The hybrid solver.** A `solver:` block emits `algo.maxwell_solver = hybrid` and the
   `hybrid_pic_model.*` parameters, together with the three laser-deposition swaps.

Four things found in the doing, each of which would have produced a *running* simulation of
the wrong problem:

- **The operator's tokens are `species` | `electron_fluid`, not `particles` | `fluid`.**
  The schema now uses the operator's own spelling rather than a friendlier synonym — a
  translation layer is one more place for the deck to drift from what WarpX parses.
- **A hybrid deck must emit no electron macroparticles at all.** The Ohm's-law solver forms
  `J_e` by subtraction, so a stray electron species is counted as an ion and subtracted
  twice. Nothing crashes.
- **`laser_deposition.species` must be OMITTED, not emptied,** in the fluid path: WarpX
  aborts on a species list nothing would read, because a stale list is how a deck comes to
  claim it heats something it does not.
- **A collisions pair naming a species the deck never creates** aborts at WarpX startup —
  i.e. after the queue has handed over the GPU. Pairs are now validated at config time
  against the species that will actually be emitted.

Both are in the `--verify` parse-back list, which is not optional: a stale binary silently
ignoring `refraction = 0` cost 2000 steps of a previous run (CLAUDE.md).

**Gate handling.** A hybrid run has no electron macroparticles, so the `ω_pe` and Debye
constraints do not *exist* for it. G1/G2 report `n/a` rather than `pass` — `pass` would read
as "checked and fine", which is a different claim. G3 likewise: grid heating is a
macroparticle effect, so a laser-off control does not buy there what it buys elsewhere.
...
| `P4_lez_kin_off` | G3 control: identical, `intensity = 0` | ~0.09 GPU-h |
