# P0_rear_reflect — is the region behind the target removable? (TAMPED / semi-infinite)
...
**Expected.** The rear rarefaction is reflected back into the slab, so its momentum is returned rather than carried away. Expect the front-side piston to be **stronger** than the full-domain reference if the two sides are coupled.
...
`geometry.boundary.axis.lo: open → reflecting`. Everything else — target, laser, field, `dz`,
...
Ran 24 000 steps (2.348 ps) in 4.7 min at 4 threads. `--verify` OK.

**VERDICT: not valid as a truncation — it is different physics.** The reflecting rear wall
returns the rear rarefaction's momentum to the slab instead of letting it leave, and that
**flips the sign of the target's net momentum**: total target-ion `p_z` is **+0.0067** here
against **−0.0315** for `P0_bc_open_B` (and −0.0305 for `P0_rear_open`). In the full domain
the rear blowoff dominates and the target as a whole recoils *away* from the laser; with a
tamped rear it does not.

That is exactly the coupling predicted before running (see **Expected** above), and it means
a reflecting rear models a **tamped or semi-infinite target** — a legitimate but *different*
configuration, not a cheaper version of the free-standing foil.

Front-side agreement is otherwise decent — target-ion count at z > −40 is +0.3 %, `n_e(z)`
median 4.8 %, `E_abs` −5.6 %, plume front within 1.5 d_e — so the discriminator is the
momentum balance, not the local plume. Had only the front-side density been checked, this
run would have looked equivalent.

**Use `P0_rear_open` for truncation.** Keep this run as the reference for what a tamped
target does, should that ever be wanted deliberately.

*(The `f_abs(0)` offset of +9.9 % is noise — see `studies/fabs_noise/`, 10.4 % 1σ. And the
`p_z(front side)` figures printed by `compare_frontside.py` are a signed sum that is only
32–44 % of the gross flux, so they carry large cancellation noise; the total-momentum
comparison above is the reliable one.)*
