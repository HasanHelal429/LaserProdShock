# P4_lez_kin_mr100_lam4x — wavelength alone — isolating the λ exponent

**Phase.** 4, `TEST_PLAN.md` §12
**Question.** What is `d(ln f_abs)/d(ln λ)`? It is the one exponent in the family never measured, and the companion leg cannot be read without it.
**Expected.** `T_ss ∝ λ^(4/3)` rises 6.96×, so plume `T_e` should rise steeply. `⟨f_abs⟩` is the measurement.
...
| `laser.intensity` | 1.0e17 W/m² | unchanged at 1e13 W/cm² |
