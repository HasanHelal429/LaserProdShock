# P5_flashic_t02 — handoff-time ladder, rung `t₀ = 0.2 ns`
...
**Expected.** Over the shared window 0.2 → 1.0 ns the rungs are the same physical problem
...
**What is different from `P5_flashic`:** `ic_flash.yaml` is fitted to FLASH's 0.2 ns
plotfile, and `max_step` = 8124000 so the run still ends at `t_FLASH` = 1.0 ns
(803.1 ps of WarpX time). Nothing else.

**Cost.** Strictly cheaper than the spine — this rung is 803.1 ps against 903.4.
...
1D  |  propagation axis z  |  lengths in d_e at critical density = 0.1693 um
  ...
  grid              : 22040 cells, dz = 0.5 d_e, dt = 0.09885 fs, 8124000 steps = 803.1 ps
```
