...
| `flash_ic.py` | **The FLASH handoff state** — the four profiles that become the PIC initial condition (`n_e`, `T_e`, `T_i`, `v_z`) at t = 0.1 ns, with the density the decks cap away shaded and, with `--overlay <run_dir>`, the WarpX config's fitted IC dashed on top. `--time`, `--zlim`, `--units zeta\|um` | FLASH delivery (+ optional run dir) | `media/P4/P4_lez_flash/flash_ic.png` | **built** |
| `mass_ratio_scan.py` | **The µ-sweep** — plume `T_e` for mr25/mr100/mr400/**mrreal** on each leg's own ζ, against `µ^(1/3)`, plus the raw-eV FLASH/PSC/WarpX comparison the real-mass leg makes possible | run dirs under `runs/P4/` | `media/P4/P4_lez_kin_mrreal/mass_ratio_scan.png` | **built** |
...
| `flash_absorption.py` | **FLASH's `f_abs` on the PIC legs' own convention.** `DELIVERY.md`'s 87.04 % is a WHOLE-RUN figure and no PIC leg runs the 0-0.1 ns ramp; window-matched, FLASH absorbs 0.6823 over 0.1-0.3 ns. `--against <run_dir>` prints the ratio and the `f_abs^(2/3)` temperature correction, and REFUSES a reduced-mass leg | FLASH delivery (+ optional run dir) | stdout | **built 2026-08-29** |
| `ic_optical_depth.py` | **Does the fitted corona absorb like FLASH's?** Integrates the same IB kernel along both handoff profiles; separates FLASH's grid (1.69x -- no cells in 0.9 < n/n_cr < 1) from the IC fit's shape (1.80x). Needs the `physics` env | FLASH delivery + run dir | stdout | **built 2026-08-29** |
| `xcode_trajectory.py` | **The benchmark as a ratio-versus-tau CURVE**, with a fitted `dlnR/dtau` so "flat" is measured, not eyeballed. `--g3` subtracts the laser-off control's heating, `--band` sets the seed error bar. Real-mass legs only. Needs the `physics` env | run dir + FLASH delivery | `media/xcode/trajectory.png`, `--csv` | **built 2026-08-29** |
